...
const SAMPLE_FORMS: Form[] = [

  {
    ...
    description: 'Collects demographics, insurance, and emergency contact details for first-time patients.',
...
const FacilityFolders: React.FC = () => {
  ...
  const [sendModal, setSendModal] = useState<{ form: Form; facility: Facility } | null>(null)



  const totalForms = facilities.reduce((sum, f) => sum + (f.forms?.length ?? 0), 0)
  ...
  useEffect(() => {
    ...
    const fetchFacilities = async () => {
      ...
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/Admin`)
        ...
        const mapped: Facility[] = (Array.isArray(data) ? data : []).map((item: any) => ({
  ...
  const filtered = useMemo(() => {
    ...
    return facilities
      ...
      .filter(fac => fac.officeName.toLowerCase().includes(q) || (fac.forms?.length ?? 0) > 0)
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                ...
                Select a facility folder and send forms directly to your patients
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl shadow-sm">
                ...
                <span className="text-sm font-bold text-gray-700">{facilities.length}</span>
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl shadow-sm">
                ...
                <span className="text-sm font-bold text-gray-700">{totalForms}</span>
          ...
          {loading ? (
            ...
            filtered.map((facility: Facility, i: number) => (
              ...
              <FacilityFolder
                ...
                onSendForm={(form, fac) => setSendModal({ form, facility: fac })}
